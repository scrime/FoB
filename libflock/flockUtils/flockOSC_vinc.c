...
main (int argc, char ** argv)
{
  ...
  // max nb of measure to store before exiting
  // automatically from the loop
  int max_nb_count = 100000;

  // for measures of 3 positions + 3 angles
  // measures = malloc(6*number_of_birds*sizeof(float));
  // doing in statically now...
  float measures[1200000]; // that is 100000 measures
  ...
  output_filename = argv[optind++]; //file to get the measures
  printf("Measures will be recorded in file %s", output_filename);
  ...
  //BOUCLE PRINCIPALE d'AQUISITION
  while (!terminate)
    {
      int change_set_to;
      fd_set read_fd_set;
      ...
      if (FD_ISSET (flockfd, &read_fd_set))
	{
	  ...
	  /* Get data from flock. */
	  if ((flock_next_record (flock, 1) == 0) || (count >= max_nb_count))
	    {
	      result = EXIT_FAILURE;
	      goto terminate;
	    }
	  ...
	  for (bird = 0, data = data_of_birds;
	       ...
	       bird++, data++)
	    {
		  ...
		  //store in memory for dumping at the end of loop
	      measures[12*(count-1) + 6*bird  ] = data->rec.values.pa.x;
	      measures[12*(count-1) + 6*bird+1] = data->rec.values.pa.y;
	      measures[12*(count-1) + 6*bird+2] = data->rec.values.pa.z;
	      measures[12*(count-1) + 6*bird+3] = data->rec.values.pa.xa;
	      measures[12*(count-1) + 6*bird+4] = data->rec.values.pa.ya;
	      measures[12*(count-1) + 6*bird+5] = data->rec.values.pa.za;
 ...
 terminate:
  ...
  if (((streamL =fopen(*output_filename+'L', 'wb')) != NULL)
	&&((streamR =fopen(*output_filename+'R', 'wb')) != NULL))
	{
	  // this -4 is to be sure we do not exceed array... to be removed///
	  for (i=0; (i<(max_nb_count-4));i = i+12)
		fprintf(streamL, "%f\t%f\t%f\t%f\t%f\t%f\n", measures[i], measures[i+1], measures[i+2], measures[i+3], measures[i+4], measures[i+5]);
	  for (i=1;(i<(max_nb_count-4));i = i+12)
		{
		  fprintf(streamR, "%f\t%f\t%f\t%f\t%f\t%f\n", measures[i], measures[i+1], measures[i+2], measures[i+3], measures[i+4], measures[i+5]);
		  fclose(streamL);
		  fclose(streamR);
		}
	}
