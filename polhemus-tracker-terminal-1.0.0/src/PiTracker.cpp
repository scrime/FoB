...
int PiTracker::WriteUsbData(void* data,int len){
  ...
  int res = libusb_bulk_transfer(m_handle,m_usbWriteEp,(BYTE*)data,len,&bw,50);

  switch (res) {
  case 0: break;
  case LIBUSB_ERROR_TIMEOUT:
    printf ("WriteUsbData: LIBUSB_ERROR_TIMEOUT\n");
    break;
  case LIBUSB_ERROR_PIPE:
    printf ("WriteUsbData: LIBUSB_ERROR_PIPE\n");
    break;
  case LIBUSB_ERROR_OVERFLOW:
    printf ("WriteUsbData: LIBUSB_ERROR_OVERFLOW\n");
    break;
  case LIBUSB_ERROR_NO_DEVICE:
    printf ("WriteUsbData: LIBUSB_ERROR_NO_DEVICE\n");
    break;
  default:
    printf ("WriteUsbData: UNKNOWN ERROR\n");
    break;
  }
...
int PiTracker::ReadUsbData(void* buf,int maxlen){
  ...
  if (m_FtContUsb){
    ...
    int res = libusb_bulk_transfer(m_handle,m_usbReadEp,(BYTE*)buf,maxlen,&br,50);

    switch (res) {
    case 0: break;
    case LIBUSB_ERROR_TIMEOUT:
      printf ("ReadUsbData: LIBUSB_ERROR_TIMEOUT\n");
      break;
    case LIBUSB_ERROR_PIPE:
      printf ("ReadUsbData: LIBUSB_ERROR_PIPE\n");
      break;
    case LIBUSB_ERROR_OVERFLOW:
      printf ("ReadUsbData: LIBUSB_ERROR_OVERFLOW\n");
      break;
    case LIBUSB_ERROR_NO_DEVICE:
      printf ("ReadUsbData: LIBUSB_ERROR_NO_DEVICE\n");
      break;
    default:
      printf ("ReadUsbData: UNKNOWN ERROR: %d\n", res);
      break;
    }
  }
